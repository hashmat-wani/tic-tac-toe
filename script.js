...
selectBtnO.onclick = () => {
  selectBox.classList.add("hide"); //hide select box
  playBoard.classList.add("show"); //show the playboard section
  players.setAttribute("class", "players active player_O"); //set class attribute in players with players active player values
  selectedPlayer = "O";
};

// user click function
function clickedBox(element) {
  if (players.classList.contains("player_O")) {
    ...
    players.classList.remove("active"); //removing active class in players
  ...
  if (runBot) {
    ...
    // toggleplayer O to X and viceverse
    players.classList.toggle("player_O");
  }
  if (selectWinner()) return; //calling selectWinner function
...
function selectWinner() {
  ...
  if (
    ...
    } else if (playerSign === selectedPlayer) {
      wonText.innerHTML = `Hurray, you won the game!`;
    } else {
      wonText.innerHTML = `Sorry, you lose the game!`;
    }
    return true;
  } else if (
    getIdVal(1) != "" &&
